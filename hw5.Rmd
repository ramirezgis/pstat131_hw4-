...
augment(ridge_final_fit, new_data = pokemon_test) %>%
  rsq(truth = type_1, estimate = .pred) #rsq??? not roc_auc or multi_metric
```


Exercise 8 - HELP
```{r}
bug_fit <- augment(ridge_final_fit, new_data = pokemon_test) %>%
  roc_curve(type_1, .pred_Bug) %>%
  autoplot()

fire_fit <- augment(ridge_final_fit, new_data = pokemon_test) %>%
  roc_curve(type_1, .pred_Fire) %>%
  autoplot()

grass_fit <- augment(ridge_final_fit, new_data = pokemon_test) %>%
  roc_curve(type_1, .pred_Grass) %>%
  autoplot()

normal_fit <- augment(ridge_final_fit, new_data = pokemon_test) %>%
  roc_curve(type_1, .pred_Normal) %>%
  autoplot()

water_fit <- augment(ridge_final_fit, new_data = pokemon_test) %>%
  roc_curve(type_1, .pred_Water) %>%
  autoplot()

psychic_fit <- augment(ridge_final_fit, new_data = pokemon_test) %>%
  roc_curve(type_1, .pred_Psychic) %>%
  autoplot()

heatmap(pokemon_factor, scale = "column")
```
